...
export declare class StalcraftAppClient extends StalcraftClient {
    ...
    getAuctionPriceHistory(regionId: string, itemId: string, parameters?: {
        offset?: number;
        limit?: number;
    }): Promise<AuctionPriceHistory>;
    getAuctionLots(regionId: string, itemId: string, parameters?: {
        offset?: number;
        limit?: number;
        sort?: Sort;
        order?: Order;
    }): Promise<AuctionLots>;
    getClanInformation(regionId: string, clanId: string): Promise<ClanInfo>;
    getClansList(regionId: string, parameters?: {
        offset?: number;
        limit?: number;
    }): Promise<ClansList>;
    getEmissionInfo(regionId: string): Promise<EmissionInfo>;
}
export declare class StalcraftUserClient extends StalcraftAppClient {
    constructor(url: string, token: string);
    getCharactersList(regionId: string): Promise<CharacterInfo[]>;
    getClanMembers(regionId: string, clanId: string): Promise<ClanMember[]>;
    getListOfFriends(regionId: string, characterName: string): Promise<string[]>;
}
export {};
